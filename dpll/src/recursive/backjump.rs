//! Augments the [`Backjump` solver][super::Backjump] with backjumping.
...
implem! {
    impl(Lit, F: Formula<Lit = Lit>) for Backjump<Lit> {
        From<F> {
            |f| Self::new(f),
        }
    }
    impl(Lit) for Backjump<Lit> {
        Deref<Target = Γ<Lit>> {
            |&self| &self.γ,
            |&mut self| &mut self.γ,
        }
        ...
        'conj_iter: for lclause in self.δ.iter() {
            ...
            if new_clause.is_empty() {
                ...
                if new_clause.len() == 1 {
                    ...
                    // {
                    //     let nlit = lit.ref_negate();
                    //     if let Some(nlit_deps) = new.γ.get(&nlit) {
                    //         new_deps.extend(nlit_deps.iter().cloned());
                    //         raise!(unsat new_deps)
                    //     }
                    // }
